...
int save_without_exif(char *file_name, long img_size, long exif_start,
                      BYTE *img_bytes);
...
int save_without_exif(char *file_name, long img_size, long exif_start,
                      BYTE *img_bytes) {
  long exif_data_size =
      ((img_bytes[exif_start + 2] << 8) | img_bytes[exif_start + 3]) +
      2;  // + 2 to include exif marker 0xFFE1
  long i = exif_start + exif_data_size;

  while (!(img_bytes[i] == 0xFF && img_bytes[i + 1] == 0xDB)) {
    i += ((img_bytes[i + 2] << 8) | img_bytes[i + 3]) +
         2;  // skip to next marker
  }
...
void init_basic_jfif(BYTE purged[]) {
  ...
  purged[6] = 0x4A, purged[7] = 0x46, purged[8] = 0x49, purged[9] = 0x46,
  purged[10] = 0x00;
  ...
  // http://web.archive.org/web/20200929185628/https://www.ccoderun.ca/programming/2017-01-31_jpeg/
  // if necessary
}