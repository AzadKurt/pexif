...
long validate_exif_sig(long byte_count, BYTE img_bytes[byte_count])
{
    ...
    for (long i = 2; i < byte_count - 1; i++)
    {
        if (img_bytes[i] == 0xFF)
        {
            ...
            // if another marker that is not the exif marker is present, jump forward by the specified data size of the current marker
            if ((marker & 0xF0) == 0xE0)
            {
                i += ((img_bytes[i + 2] << 8) | img_bytes[i + 3]) + 1;
                // + 1 because the two bytes that specify the data size are included and + 1 together with i++ results in an increase of 2
            }
        }
    }
    return 0;
...
char *get_pexifed_file_name(char *file_name)
{
    ...
    int relevant_idx = last_backslash_idx != -1 ? last_backslash_idx : last_slash_idx;
